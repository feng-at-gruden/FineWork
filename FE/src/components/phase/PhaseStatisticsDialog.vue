        ...
        <v-card>
            ...
            <v-container class="statistics-container">
                <v-layout wrap>
                    ...
                    <v-flex xs4 md3 class="task-count-container">
                        ...
                        <v-progress-linear color="green" height="5" :value="progress(data.TaskCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>已完工</label>
                        <span class="task-count">{{data.FinishedCount}}</span>
                        <v-progress-linear color="green" height="5" :value="progress(data.FinishedCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>未完工</label>
                        <span class="task-count">{{data.UnfinishedCount}}</span>
                        <v-progress-linear color="grey" height="5" :value="progress(data.UnfinishedCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>施工中</label>
                        <span class="task-count">{{data.WorkingCount}}</span>
                        <v-progress-linear color="blue" height="5" :value="progress(data.WorkingCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>提前完工</label>
                        <span class="task-count">{{data.FulfilAheadCount}}</span>
                        <v-progress-linear color="green" height="5" :value="progress(data.FulfilAheadCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>延期未开工</label>
                        <span class="task-count">{{data.DelayedCount}}</span>
                        <v-progress-linear color="red" height="5" :value="progress(data.DelayedCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>逾期未完工</label>
                        <span class="task-count">{{data.ExceededCount}}</span>
                        <v-progress-linear color="red" height="5" :value="progress(data.ExceededCount)"></v-progress-linear>
                    </v-flex>
                    <v-flex xs4 md3 class="task-count-container">
                        <label>停工中</label>
                        <span class="task-count">{{data.PendingCount}}</span>
                        <v-progress-linear color="red" height="5" :value="progress(data.PendingCount)"></v-progress-linear>
                    </v-flex>
                </v-layout>
            </v-container>
            <div class="text-xs-center">
                ...
                <v-card-actions class="pa-4">
                    <v-spacer></v-spacer>
                    <v-btn flat @click="dialog = false" :loading="loading">关闭</v-btn>
                    <v-spacer></v-spacer>
                </v-card-actions>
            </div>
        </v-card>
